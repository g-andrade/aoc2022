...
_opponent_played_rock:
	CMP R1, #'X' @ we need to lose
	BEQ _rock_vs_scissors
	CMP R1, #'Y' @ we need to draw
	BEQ _rock_vs_rock
	B _rock_vs_paper @ we need to win
_rock_vs_rock:
	MOV R0, #(3 + 1)
...
_opponent_played_paper:
	...
	CMP R1, #'Y' @ we need to draw
	BEQ _paper_vs_paper
	B _paper_vs_scissors @ we need to win
_paper_vs_rock:
	MOV R0, #(0 + 1)
	B _finished_calculating_score_increment
_paper_vs_paper:
...
_opponent_played_scissors:
	...
	CMP R1, #'Y' @ we need to draw
	BEQ _scissors_vs_scissors
	B _scissors_vs_rock @ we need to win
_scissors_vs_rock:
	MOV R0, #(6 + 1)
	B _finished_calculating_score_increment
_scissors_vs_paper:
